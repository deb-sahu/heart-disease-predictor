...
from src.data_loader import get_feature_target_split, prepare_dataset
from src.preprocessing import convert_target_to_binary, create_preprocessor, handle_missing_values, save_preprocessor
...
def get_models() -> Dict[str, Any]:
    ...
    """
    return {
        "LogisticRegression": LogisticRegression(max_iter=1000, random_state=RANDOM_STATE, solver="lbfgs"),
        "RandomForest": RandomForestClassifier(n_estimators=200, max_depth=5, random_state=RANDOM_STATE, n_jobs=-1),
    }
...
def plot_confusion_matrix(y_true: pd.Series, y_pred: np.ndarray, model_name: str) -> str:
    """
...
def plot_roc_curve(y_true: pd.Series, y_prob: np.ndarray, model_name: str) -> str:
    """
...
def train_and_log_model(
    ...
    with mlflow.start_run(run_name=f"{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        mlflow.sklearn.log_model(model, f"model_{model_name}", signature=signature, input_example=input_example)
...
def train_all_models() -> Dict[str, Any]:
    ...
    for model_name, model in models.items():
        trained_model, metrics = train_and_log_model(
            model_name, model, X_train_processed, X_test_processed, y_train, y_test, preprocessor
        )
    ...
    return {"results": results, "best_model_name": best_model_name, "best_metrics": best_metrics}
