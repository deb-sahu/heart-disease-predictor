...
class TestPreprocessing:
    ...
    def test_handle_missing_values_no_missing(self, sample_dataframe):
        ...
        pd.testing.assert_frame_equal(df_cleaned.reset_index(drop=True), sample_dataframe.reset_index(drop=True))
    ...
    def test_convert_target_already_binary(self):
        ...
        df = pd.DataFrame({TARGET_COLUMN: [0, 1, 0, 1, 1]})
    ...
    def test_preprocess_data_transform_only(self, sample_dataframe):
        ...
        X_processed, y, _ = preprocess_data(df_new, fit=False, preprocessor=fitted_preprocessor)
